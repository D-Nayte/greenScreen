...
const writeEnvSensorData = async () => {
  ...
  config.generall.humidityAir.current = parseFloat(
    sensorData?.humidity.toFixed(2)
  );
  config.generall.temperature.current = parseFloat(
    sensorData?.temperature_C.toFixed(2)
  );
  config.generall.pressure.current = parseFloat(
    sensorData?.pressure_hPa.toFixed(2)
  );
